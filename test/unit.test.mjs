...
test('config still rejects 0 for backend timeout', () => {
  const prev = process.env.NUVIO_BACKEND_TIMEOUT_MS;
  process.env.NUVIO_BACKEND_TIMEOUT_MS = '0';
  try {
    assert.equal(loadConfig().backendTimeoutMs, 30000);
  } finally {
    if (prev === undefined) delete process.env.NUVIO_BACKEND_TIMEOUT_MS;
    else process.env.NUVIO_BACKEND_TIMEOUT_MS = prev;
  }
});
